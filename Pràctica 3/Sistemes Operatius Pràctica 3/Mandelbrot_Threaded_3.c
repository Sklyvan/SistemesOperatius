...
typedef struct LockMatrix{
    pthread_mutex_t matrix[R][R];
    bool statusMatrix[R][R];
} LockMatrix;

void initializeLockMatrix(LockMatrix* lockMatrix){
    for (int i=0; i<R; i++){
        for (int j=0; j<R; j++){
            lockMatrix->statusMatrix[i][j] = false;
            pthread_mutex_init(&lockMatrix->matrix[i][j], NULL);
        }
    }
}

bool isLocked(LockMatrix* lockMatrix, int i, int j){
    pthread_mutex_lock(&GlobalLock);
    bool toReturn =  lockMatrix->statusMatrix[i][j];
    pthread_mutex_unlock(&GlobalLock);
    return toReturn;
}

typedef struct thread2_args{
    unsigned char* pixels; LockMatrix* lockMatrix;
} thread2_args;
...
void* threadedInterchange(void* args){
    unsigned char* pixels = ((struct thread2_args*)args)->pixels;
    pthread_mutex_lock(&GlobalLock);
    LockMatrix* lockMatrix = ((struct thread2_args*)args)->lockMatrix;
    pthread_mutex_unlock(&GlobalLock);
    for (int i=0; i<1000/N; i++){ // Esta función la ejecuta cada thread, así que dependiendo del número de threads, hacemos una cantidad determinada de intercambios.
        int si = rand()%R; int sj = rand()%R; // Cuadrado Inicial.
        int ti = rand()%R; int tj = rand()%R; // Cuadrado Final.
        if (si != ti || sj != tj ){ // Si esto no se cumple, el intercambio se está haciendo del cuadrado (a, b) al cuadrado (a, b), por tanto, no vale la pena hacer nada.
            int source1D = sj*R + si; int target1D = tj*R + ti; // Esto nos permite ver nuestra matrix con 1D, y de esta forma podemos saber 'que va primero'.
            // Con esto, evitamos el problema de que el Thread1 quiera hacer lock de la región A y la región B, mientras que el Thread2 quiere hacer lock de la región B y la región A.
            // Si el Thread1 hace el Lock_A y se ve interrumpido, luego el Thread2 puede hacer el Lock_B, y de esta forma, ni el Thread1 podrá iniciar el Lock_B ni el Thread2 el Lock_A. (Deadlock)
            if(source1D < target1D){ pthread_mutex_lock(&lockMatrix->matrix[si][sj]); pthread_mutex_lock(&lockMatrix->matrix[ti][tj]); }
            else if(target1D < source1D){ pthread_mutex_lock(&lockMatrix->matrix[ti][tj]); pthread_mutex_lock(&lockMatrix->matrix[si][sj]); }
            interchange(si, sj, ti, tj, pixels, width, height); // Ejecutamos la operación 'delicada'.
            if(source1D < target1D){ pthread_mutex_unlock(&lockMatrix->matrix[si][sj]); pthread_mutex_unlock(&lockMatrix->matrix[ti][tj]); }
            else if(target1D < source1D){ pthread_mutex_unlock(&lockMatrix->matrix[ti][tj]); pthread_mutex_unlock(&lockMatrix->matrix[si][sj]); }
        }
    }
    return NULL;
}
...
void setPixel(unsigned char* pixel, int x, int y){
    ...
    *(pixel+1) = 255 * ((float)y / width); // Green Channel.
    // IMPORTANTE! Intencambiamos la posición x, y al calcular la iteración, ya que si no lo hacemos, el dibujo sale rotado 90º.
...
int main(void) {
    ...
    LockMatrix MyLockMatrix;
    initializeLockMatrix(&MyLockMatrix);
    thread2_args args = {.pixels = pixels, .lockMatrix = &MyLockMatrix};
    startTimer(0);
    for(int i=0; i<N; i++) pthread_create(&tids2[i],NULL, threadedInterchange, &args);
