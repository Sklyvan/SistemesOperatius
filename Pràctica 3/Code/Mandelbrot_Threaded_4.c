...
typedef struct MonitorMatrix{
    monitor_lock matrix[R][R];
    bool statusMatrix[R][R];
} MonitorMatrix;

void initializeMonitorMatrix(MonitorMatrix* monitorMatrix){
    for (int i=0; i<R; i++){
        for (int j=0; j<R; j++){
            monitorMatrix->statusMatrix[i][j] = false;
            mon_lock_init(&monitorMatrix->matrix[i][j]);
        }
    }
}

bool isLocked(MonitorMatrix* monitorMatrix, int i, int j){
    pthread_mutex_lock(&GlobalLock);
    bool toReturn =  monitorMatrix->statusMatrix[i][j];
    pthread_mutex_unlock(&GlobalLock);
    return toReturn;
}

typedef struct thread2_args{
    unsigned char* pixels; MonitorMatrix* monitorMatrix;
} thread2_args;
...
void* threadedInterchange(void* args){
    ...
    pthread_mutex_lock(&GlobalLock);
    MonitorMatrix * monitorMatrix = ((struct thread2_args*)args)->monitorMatrix;
    pthread_mutex_unlock(&GlobalLock);
    for (int i=0; i<1000/N; i++){ // Esta función la ejecuta cada thread, así que dependiendo del número de threads, hacemos una cantidad determinada de intercambios.
        ...
        if (si != ti || sj != tj ){ // Si esto no se cumple, el intercambio se está haciendo del cuadrado (a, b) al cuadrado (a, b), por tanto, no vale la pena hacer nada.
            ...
            if(source1D < target1D){ mon_lock(&monitorMatrix->matrix[si][sj]); mon_lock(&monitorMatrix->matrix[ti][tj]); }
            else if(target1D < source1D){ mon_lock(&monitorMatrix->matrix[ti][tj]); mon_lock(&monitorMatrix->matrix[si][sj]); }
            interchange(si, sj, ti, tj, pixels, width, height); // Ejecutamos la operación 'delicada'.
            if(source1D < target1D){ mon_unlock(&monitorMatrix->matrix[si][sj]); mon_unlock(&monitorMatrix->matrix[ti][tj]); }
            else if(target1D < source1D){ mon_unlock(&monitorMatrix->matrix[ti][tj]); mon_unlock(&monitorMatrix->matrix[si][sj]); }
        }
    }
    return NULL;
}
...
int main(void) {
    ...
    MonitorMatrix MyMonitorMatrix;
    initializeMonitorMatrix(&MyMonitorMatrix);
    thread2_args args = {.pixels = pixels, .monitorMatrix = &MyMonitorMatrix};
    startTimer(0);
